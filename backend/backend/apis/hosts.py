...
@api.get("/register", tags=["Hosts"])
def register_host(host: Host, host_service: HostService = Depends()) -> JSONResponse:
    try:
    ...
    return JSONResponse(status_code=status.HTTP_201_CREATED, content={"message": "Host registered successfully."})